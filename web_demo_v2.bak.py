...
# 2 A10 尝试分配
device_map = infer_auto_device_map(model, max_memory={0:'18GiB',1:'20GiB','cpu':'18GiB'}, no_split_module_classes='CogVLMDecoderLayer')
...
def process_video(filepath_path):
    videoCapture = cv2.VideoCapture(filepath_path)
...
def web_video_chat(file_path,
    ...
    if no_prompt:
        query = ''
    image_list = process_video(file_path)
...
if __name__ == '__main__':
    ...
    with gr.Blocks() as demo:
        ...
        input_text.submit(fn=http_post,inputs=[input_text, temperature, top_p, top_k, file_prompt, result_text, hidden_image_hash],
                            outputs=[input_text, result_text, hidden_image_hash])
        clear_button.click(fn=clear_fn, inputs=clear_button, outputs=[input_text, result_text, file_prompt])
        file_prompt.upload(fn=clear_fn2, inputs=clear_button, outputs=[result_text])
        file_prompt.clear(fn=clear_fn2, inputs=clear_button, outputs=[result_text])

        print(gr.__version__)

    demo.queue(concurrency_count=10)
    # demo.launch(share=True)
    demo.launch(server_name="0.0.0.0", show_error=True, server_port=7861)
