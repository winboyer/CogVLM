...
def parse_query_response(query_str):
    chinese_query_flag = False
    verb_noun_list = []
    det_list = []
    for _char in query_str:
        if '\u4e00' <= _char <= '\u9fa5':
            print('this is chinese sentence')
            chinese_query_flag = True
            break
    if chinese_query_flag:
        doc_result = nlp_zh(query_str)
    else:
        doc_result = nlp(query_str)
    
    for chunk in doc_result:
        # print(chunk.text, chunk.pos_)
        if chunk.pos_ in ('NOUN', 'VERB'):
            verb_noun_list.append(chunk.text)
        elif chunk.pos_ in ('DET'):
            det_list.append(chunk.text)
            
    return verb_noun_list, det_list
