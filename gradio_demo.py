...
if __name__ == '__main__':
    ...
    with gr.Blocks() as demo:
        ...
        #                         inputs=[input_text, video_prompt],
        #                         label="Example Inputs (Click to insert an example into the input box)",
        #                         examples_per_page=6)
        
        print(gr.__version__)
        # run_button.click(fn=chat_pred,inputs=[input_text, temperature, top_p, top_k, video_prompt, result_text, hidden_image_hash],
        #                     outputs=[input_text, result_text, hidden_image_hash])
        input_text.submit(fn=clear_fn2,inputs=[input_text, temperature, top_p, top_k, video_prompt, result_text, hidden_image_hash],
                            outputs=[input_text, result_text, hidden_image_hash])
